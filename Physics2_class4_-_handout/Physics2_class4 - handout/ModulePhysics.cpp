...
update_status ModulePhysics::PostUpdate()
{
	...
	if (body_found && App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT)
	{
		App->renderer->DrawLine(mouse_joint->GetAnchorA().x, mouse_joint->GetAnchorA().y, mouse.x, mouse.y, 255, 255, 0);
		mouse_joint->SetTarget(mouse);
	}

	// TODO 4: If the player releases the mouse button, destroy the joint
	if (body_found && App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_UP)
	{
